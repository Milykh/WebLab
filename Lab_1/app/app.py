...
comments = generate_comments()# Генерим комментарий
...
def post(index):
    ...
    return render_template('post.html', title=p['title'], post=p, comments=comments)# передаём посты и комментарии
...
if __name__ == "__main__":# как всегда добавляем эту конструкцию
    app.run()